...
- (void)doTestStacktrace:(NSString *)stackTraceString forFQClassName:(NSString *)fqClassName
	...
	GHAssertTrue([[item fullClassName] isEqualToString:fqClassName], 
		@"FQClassname should be %@, but is %@", fqClassName, [item fullClassName]);
	GHAssertTrue([[item method] isEqualToString:method], 
		@"Method should be %@, but is %@", method, [item method]);
	GHAssertTrue([[item file] isEqualToString:file], 
		@"Filename should be %@, but is %@", file, [item file]);
	GHAssertEquals((int)[item line], (int)line, 
