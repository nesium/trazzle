...
@interface ZZAppDelegate (Private)
- (void)_loadPlugins;
...
- (void)_loadPlugins{
	...
	for (NSString *path in plugins){
		...
		if (prinClass && class_conformsToProtocol(prinClass, @protocol(ZZTrazzlePlugIn))){
			ZZPlugInController *plugInController = [[ZZPlugInController alloc] 
